...
const SignupStudent = () => {
  const [formData, setFormData] = useState({
    name: '', email: '', mobile: '', rollNo: '', division: '', password: '', confirmPassword: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosInstance.post('/auth/student/signup', formData);
    ...
    <div className="flex items-center justify-center min-h-[calc(100vh-64px)] py-10 px-4">
      <div className="glass-card w-full max-w-lg animate-fade-in">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">Student Signup</h2>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
            ...
            <input name="name" type="text" value={formData.name} onChange={handleChange} className="input-field" placeholder="John Doe" required />
          </div>
          <div>
          ...
          <div>
            <label className="block text-slate-700 text-sm font-semibold mb-1.5 ml-1">Roll Number</label>
            <input name="rollNo" type="text" value={formData.rollNo} onChange={handleChange} className="input-field" placeholder="A-101" required />
          </div>
          <div>
            <label className="block text-slate-700 text-sm font-semibold mb-1.5 ml-1">Division</label>
            <input name="division" type="text" value={formData.division} onChange={handleChange} className="input-field" placeholder="B" required />
          </div>
          <div>
